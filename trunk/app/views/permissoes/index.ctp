...
<div class="toolbar">
	<?php echo $html->link(__('Nova Permissão',true), 'add',array('class'=>'linkbutton linkbtn btn_add')); ?>	<a href="#" onclick="location.href='<?php echo $this->webroot;?>/permissoes/deleteselected/'+selecionados()" class="linkbutton linkbtn btn_delete">Excluir Vários</a>
	...
	if($total!=1){
		...
		echo '<select style="height:22px;font-size:13px" onchange="location.href=\''. $this->webroot.'permissoes/index/page:\'+this.value+\''.$sort.$direction.'\'">';
		for($x=1;$x<=$total;$x++){
...
<div id="tabpanel">
    <ul>
        <li><a href="#tab1"><span><?php echo __("Listar Permissoes",true) ?></span></a></li>
    </ul>
    <div id="tab1">
	...
	<tr<?php echo $class;?>>
						...
						<td align="center" style="padding: 0px 0px 0px 0px">
							<input type="checkbox" class="chk" value="<?=$permissao['Permissao']['id']?>">
						</td>
							<td>
			<?php echo $html->link($permissao['Permissao']['nome'], array('action'=>'view', $permissao['Permissao']['id'])); ?>
		</td>
		<td>
			<?php echo $permissao['Permissao']['descricao']; ?>
		</td>
		<td>
			<?php if($permissao['Permissao']['ativo']==1) echo 'Sim';else echo 'Não'; ?>
		</td>
	</tr>
<?php endforeach; ?>
